...
class WorkflowManager:
    ...
    def run_step1_build_db(self, is_update, db_path, source_folder_path, files_to_process, completion_callback):
        """1단계: 번역 DB 구축 또는 업데이트를 실행합니다."""
        
        # 백그라운드 스레드에서 실행될 함수
        def task():
            try:
                # 선택된 파일 목록을 전체 경로로 변환
                excel_files = []
                for file_name, full_path in files_to_process:
                    excel_files.append((file_name, full_path))
                ...
                if is_update:
                    ...
                    result = self.db_manager.update_translation_db(
                        ...
                        db_path=db_path,
                        language_list=["KR", "EN", "CN", "TW", "TH"], # 모든 언어 기준
        ...
        loading_popup = LoadingPopup(self.parent_app.root, f"DB {mode_text} 중", f"DB {mode_text} 준비 중...")
        
        thread = threading.Thread(target=task, daemon=True)
        thread.start()

# tools/workflow_manager.py의 run_step2_update_db_from_excel 함수를 아래 코드로 교체
