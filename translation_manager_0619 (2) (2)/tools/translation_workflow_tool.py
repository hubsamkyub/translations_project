...
import threading
import sqlite3
from ui.common_components import ScrollableCheckList, show_message, LoadingPopup
...
class TranslationWorkflowTool(tk.Frame):
    def __init__(self, parent, root):
        ...
        self.step3_target_folder_var = tk.StringVar()
        ...
        self.base_db_path_var.trace_add("write", self._on_db_path_change)
    ...
    def setup_ui(self):
        ...
        # [수정] 적용 대상 폴더 선택 UI 변경
        target_info_frame = ttk.Frame(self.step3_frame)
        target_info_frame.pack(fill="x", padx=5, pady=5)
        ttk.Label(target_info_frame, text="적용 대상 폴더:").grid(row=0, column=0, sticky="w", padx=5)
        # Entry가 새로운 변수를 사용하고, state를 'normal'로 변경
        ttk.Entry(target_info_frame, textvariable=self.step3_target_folder_var, state="normal").grid(row=0, column=1, sticky="ew", padx=5)
        # '찾아보기' 버튼 추가
        ttk.Button(target_info_frame, text="찾아보기", command=self.select_step3_target_folder).grid(row=0, column=2, padx=5)
        target_info_frame.columnconfigure(1, weight=1)
...
# run_step1 함수를 아래 코드로 교체합니다.
    ...
    def run_step1(self):
        ...
        # [수정] UI에서 로딩 팝업을 직접 생성하고 제어
        mode_text = "업데이트" if is_update else "구축"
        loading_popup = LoadingPopup(self.root, f"DB {mode_text} 중", f"DB {mode_text} 준비 중...")
        ...
        def completion_callback(result):
            loading_popup.close() # 작업 완료 후 팝업 닫기
            self.on_step1_complete(result)

        # 스레드에 progress_callback과 completion_callback 전달
        threading.Thread(
            target=self.workflow_manager.run_step1_build_db,
            args=(is_update, db_path, source_folder, files_to_process, progress_callback, completion_callback),
            daemon=True
        ).start()
...
# tools/translation_workflow_tool.py 파일의 TranslationWorkflowTool 클래스 내부에 아래 함수들을 추가하세요.
    ...
    def _find_headers_in_worksheet(self, ws):
        """Worksheet에서 헤더 행과 컬럼 위치 찾기 (Container -> Workflow Tool로 이동)"""
        for i in range(1, 11):
            ...
            if "STRING_ID" in cleaned_values:
                ...
                # 모든 헤더 이름을 대문자로 통일하여 반환
                return {k: v.upper() for k, v in header_map.items()}, i
        return None, -1
    
    def _on_db_path_change(self, *args):
        """기준 DB 경로가 변경되었을 때 호출되어 2단계를 활성화하는 함수"""
        db_path = self.base_db_path_var.get()
        if db_path and os.path.exists(db_path):
            # 유효한 DB 파일이 선택되면 1단계 상태를 '건너뜀'으로 표시하고 2단계를 활성화
            self.step1_status_label.config(text="✔ 기존 DB 사용 (1단계 건너뜀)", foreground="green")
            self.enable_step(self.step2_frame)
            self.log("기존 DB 파일이 선택되어 1단계를 건너뛰고 2단계를 활성화합니다.")

# TranslationWorkflowTool 클래스 내부에 아래 함수들을 추가 및 수정합니다.

    def select_step3_target_folder(self):
        """3단계의 적용 대상 폴더를 선택하는 함수"""
        folder = filedialog.askdirectory(title="최종 번역을 적용할 폴더 선택")
        if folder:
            self.step3_target_folder_var.set(folder)

    def run_step3(self):
        """3단계 최종 번역 적용 실행 (수정된 버전)"""
        base_db_path = self.base_db_path_var.get()
        # [수정] 3단계 전용 변수에서 타겟 폴더 경로를 가져옴
        target_folder_path = self.step3_target_folder_var.get()
        
        # [수정] 적용할 파일 목록을 1단계가 아닌 3단계 폴더 기준으로 다시 가져옴
        if not target_folder_path or not os.path.isdir(target_folder_path):
            show_message(self, "warning", "입력 오류", "유효한 적용 대상 폴더를 선택해야 합니다.")
            return

        # 타겟 폴더의 String 파일 목록을 다시 스캔
        files_to_process = []
        ...
        if not files_to_process:
            show_message(self, "warning", "파일 없음", "적용 대상 폴더에서 String 파일을 찾을 수 없습니다.")
            return
        ...
        apply_options = {
            ...
            "kr_match_check": self.apply_kr_match_check_var.get(),
            "kr_mismatch_delete": self.apply_kr_mismatch_delete_var.get(),
            "apply_smart_lookup": True # 스마트 적용은 항상 활성화
        }
